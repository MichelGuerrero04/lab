...
WITH lotes_potencial AS (
    SELECT qgis_id, geom, zonedist1, direccion_lote, area_planta,
           far_permitido, far_construido,
           (far_permitido - far_construido) AS far_disponible,
           (far_permitido - far_construido) * area_planta AS m2_adicionales
    FROM public.zona_analytics
    WHERE far_permitido > 0
      AND far_construido > 0
      AND (far_permitido - far_construido) > 0.5
      AND area_planta > 200
),
impacto_solar AS (
    SELECT
    ...
    JOIN public.zona_analytics v
      ON ST_DWithin(ST_Force2D(l.geom), ST_Force2D(v.geom), 25)
...
-- que violan FAR o altura cuando hay >=3 dentro de 40 m entre si.
-- eps=40m (radio de vecindad), minpoints=3 (nucleo de cluster).
-- -------------------------------------------------------
WITH viol AS (
...
clustered AS (
    SELECT qgis_id, c,
           ST_ClusterDBSCAN(c, eps := 40, minpoints := 3) OVER () AS cid
    FROM viol
)
...
-- Edificios cuya altura supera 2x la MEDIANA de sus vecinos a 50 m
-- (rupturas de escala / torres aisladas). Requiere >=5 vecinos para
-- que la mediana sea representativa.
-- -------------------------------------------------------
WITH h AS (
